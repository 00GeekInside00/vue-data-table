<template>
  <table v-if="paginatedData.length == 0">
    ...
    <thead>
      <tr>
        <th v-for="key in columns">{{ key | getDisplayName(displayNames) }}</th>
      </tr>
    </thead>
    <tbody>
      <tr>
  ...
  data() {
    ...
    if (this.data) {
      this.getColumns(
        ...
        this.data,
        this.columnsToNotDisplay
      ).forEach(key => (sortOrders[key] = 1));
    ...
    getColumns(columns, data, columnsToRemove) {
      if (columns.length === 0) {
        ...
        if (this.aggregateColumns) {
          ...
          foundColumns = allColumns.filter(
